...
const { analyzeNews, extractStockInfo } = require("./aiService");
...
async function analyzeArticleFromUrl(url, stock) {
  try {
    ...
    try {
      ...
      // 分析文章
      const analysis = await analyzeNews(article.content, stockInfo);
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.log(
          `使用axios获取到HTML内容，长度: ${htmlContent.length} 字符`
        );
      } catch (axiosError) {
        console.error("axios获取内容也失败:", axiosError);
        throw new Error(
          "无法获取页面内容: " + (axiosError.message || "未知错误")
        );
      }
    }
    ...
    if (!content) {
      ...
      console.log(
        `从${usedSelector || "body"}中提取了 ${paragraphCount} 个段落`
      );
    }
