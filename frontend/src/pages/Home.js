...
const Home = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // 分割URL，处理多个链接
    const urls = newsUrls.split(',').map(url => url.trim()).filter(url => url);
    
    if (urls.length === 0) {
      setError('请输入至少一个有效的新闻链接');
      return;
    }
  ...
  const processSingleUrl = async (url) => {
    ...
    try {
      ...
      if (symbol && companyName) {
        await performAnalysis(url, symbol, companyName);
        return;
      }
      ...
      for (let i = 0; i < urls.length; i++) {
        ...
        try {
          ...
          if (!symbol || !companyName) {
            ...
            finalSymbol = symbol || extractResponse.data.symbol;
            finalCompany = companyName || extractResponse.data.company;
          }
        ...
        <section className="form-section">
          ...
          <form onSubmit={handleSubmit}>
            <div className="form-group">
              <label htmlFor="newsUrls">新闻链接 (多个链接可用逗号分隔)</label>
              <textarea
                id="newsUrls"
                value={newsUrls}
                onChange={(e) => setNewsUrls(e.target.value)}
            ...
            <div className="form-group">
              <label htmlFor="symbol">股票代码 (选填，AI会尝试提取)</label>
              <input
                type="text"
                id="symbol"
                value={symbol}
            ...
            <div className="form-group">
              <label htmlFor="companyName">公司名称 (选填，AI会尝试提取)</label>
              <input
                type="text"
                id="companyName"
                value={companyName}
            ...
            <div className="checkbox-container">
              ...
              />
              <label htmlFor="batchMode" className="checkbox-label">批量处理模式</label>
            </div>
